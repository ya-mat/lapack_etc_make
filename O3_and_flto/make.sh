...
&& ulimit -s unlimited \
&& sed -e 's/CFLAGS = -O3/CFLAGS="-O3 -flto/g' ./make.inc > ./make.inc2 \
&& sed -e 's/FFLAGS = -O2 -frecursive/FFLAGS = -O3 -flto/g' ./make.inc2 > ./make.inc \
&& make \
&& cd ../ \
...
&& sed -i 's/FFLAGS= -O2/FFLAGS = -O3 -flto/g' ./static/makefile \
&& env FC="gfortran -std=legacy" make libslatec.a \
&& cd ../ \
...
&& sed -e 's/FFLAGS=-O2 -funroll-loops -fexpensive-optimizations/FFLAGS=-O3 -flto/g' ./Makefile2 > ./Makefile \
&& make \
&& cd ../ \
...
&& sed -e 's/OPTS      = -u -g -C/OPTS=-O3 -flto/g' ./Makefile2 > ./Makefile \
&& make \
&& cd ../ \
&& cd ../ \
